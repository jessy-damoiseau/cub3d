...
void  print_col(int x, t_parsing *parsing, t_global *global)
{
	...
	while (global->pixel.drawStart <= global->pixel.drawEnd)
	{
		if (!global->calcul.side && global->calcul.stepX == -1)
		{
...
int my_mlx_loop(t_global *global)
{
	...
	while (x < parsing->value.rx)
	{
		...
		while (global->calcul.hit == 0)
		{
			...
			if(parsing->parse.map[global->calcul.mapX][global->calcul.mapY] == '1') global->calcul.hit = 1;
		}
		...
		if(global->pixel.drawEnd >= parsing->value.ry)
      	...
      	if(global->calcul.side == 0) global->textures.wallX = global->constante.posY + global->calcul.perpWallDist * global->calcul.rayDirY;
      	else          global->textures.wallX = global->constante.posX + global->calcul.perpWallDist * global->calcul.rayDirX;
