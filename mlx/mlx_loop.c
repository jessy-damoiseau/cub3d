...
int my_mlx_loop(t_global *global)
{
	...
	while (x < parsing->value.rx)
	{
		...
		while (global->calcul.hit == 0)
		{
			...
			if (!global->sprite.sprite_on && parsing->parse.map[global->calcul.mapX][global->calcul.mapY] == '2')
			{
				global->sprite.spritemx = global->calcul.mapX;
				global->sprite.spritemy = global->calcul.mapY;
		...
		{
			if (global->sprite.sprite_on)
				global->sprite.distsprite = (global->sprite.spritemx - global->constante.posX + (1 - global->calcul.stepX) / 2) / global->calcul.rayDirX;
			global->calcul.perpWallDist = (global->calcul.mapX - global->constante.posX + (1 - global->calcul.stepX) / 2) / global->calcul.rayDirX;
		}
		else
		{
			if (global->sprite.sprite_on)
				global->sprite.distsprite = (global->sprite.spritemy - global->constante.posY + (1 - global->calcul.stepY) / 2) / global->calcul.rayDirY;
